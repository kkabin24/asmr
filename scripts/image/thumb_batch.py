...
사용법:
    python scripts/image/thumb_batch.py --out-dir _thumbs [--interval 180] [--port 3847]
    python scripts/image/thumb_batch.py --out-dir _thumbs --only 01_rainy_temple,02_bamboo_night
...
BUILD = ROOT / "docs" / "thumbnail-prompts" / "_build"
...
def main():
    ...
    ap.add_argument("--out-dir", default="_thumbs")
    ap.add_argument("--interval", type=float, default=180.0, help="장 사이 텀(초). 기본 180")
    ap.add_argument("--port", default="3847", help="flow 레인 포트(단일)")
    ...
    if not prompts:
        log("빌드된 프롬프트가 없다 — 먼저 scripts/assets/thumb_prompt.py --all")
        return 1

    out_dir = (ROOT / args.out_dir) if not os.path.isabs(args.out_dir) else pathlib.Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    env = dict(os.environ)
    env["FLOW_PORTS"] = args.port
    ...
    log(f"대상 {len(todo)}장 (완성분 건너뜀 {skipped}) · 레인 {args.port} · 텀 {interval:.0f}초 · 직렬")
