...
- **텀을 늘려서 계속 뽑지 않는다.** 남은 장을 포기하고 배치를 그 자리에서 끝낸다.
- 이유: 어뷰징 플래그가 켜진 계정에 계속 요청을 보내는 것은 §4 가 말하는 **'속도를 낮춰 예방'의 정반대**다. 일시 플래그를 영구 제재로 키우는 길이며, 배치 하나 건지려다 계정을 잃는다.
- 중단 후에는 **사람이 판단한다.** 계정을 충분히 쉬게 둔 뒤 다시 시작할지, 다른 레인으로 갈지는 자동으로 정하지 않는다.
- 재개는 안전하다 — 완성분은 SKIP(exists)로 건너뛰므로 같은 명령을 다시 돌리면 이어서 진행된다.
- 배치 러너 `scripts/image/thumb_batch.py` 에 이 동작이 내장돼 있다(감지 시 exit code 2).
...
## 24. 🚫 **[폐기 — 2026-09-06]** Veo 훅은 gemini + **Veo 3.1 Lite**로 (flow는 무료 계정에서 안 된다) ★2026-07-28 확정

> **🚫 이 절의 결론은 폐기됐다 (2026-09-06 사용자 지시 · CLAUDE.md 「절대 금지」 절).** `GEMINI_API_KEY`로는 **영상을 절대 만들지 않는다.** 훅 영상 엔진은 **무료 `pjn` 하나뿐**이고, pjn이 안 되면 **훅을 포기하고 스틸 도입으로 간다.** 아래 단가·모델 비교는 **이력 보존용**이며 실행 근거가 아니다. (flow가 무료 계정에서 안 된다는 관찰만 여전히 유효하다.)
